 ...
 * \file  ports.h
 *
 * \brief PORTA, PORTB & PORTC  Configuration Header File
 *
...
#define ODR_PORT_A    0xFF                  /* N-channel Open drain (if output) :   0 = CMOS mode,    1 = open drain*/
    ...
    // PA_SetPUP( \ | P6_NOPUP | P5_NOPUP | P4_NOPUP | P3_NOPUP | P2_NOPUP | P1_NOPUP | P0_NOPUP );
...
#define PA_SetIN( bits )          RegPAOE   &=(~bits);\
                                            RegPAInpE |= bits
...
#define PA_SetAnalog(bits)                  RegPAOE   &= ~bits;\
                                                        RegPAPU &=(~bits);\
                                                        RegPAPD &=(~bits);\
                                                        RegPAInpE &=(~bits)

#define PA_SetAnalog_with_PUP(bits)       RegPAOE   &= ~bits;\
                                                       RegPAPU |=(bits);\
                                                       RegPAInpE &=(~bits)

#define PA_SetAnalog_with_PDWN(bits)        RegPAOE   &= ~bits;\
                                                        RegPAPU &=(~bits);\
                                                        RegPAPD |=(bits);\
                                                    RegPAInpE &=(~bits)
...
#define PB_SetIN(bits)          RegPBOE   &= ~bits;\
                                          RegPBInpE |= bits

#define PB_SetOUT(bits)         RegPBOE   |= bits                 // OUTPUT  CONFIG
#define PB_SetLevel_Hi(bits)    RegPBDOut |= bits                //  OUTPUT     LEVEL: HIGH,LOW
#define PB_SetLevel_Lo(bits)    RegPBDOut &=(~bits)
...
// Use following for analog IN/OUT
#define PB_SetAnalog(bits)                  RegPBOE   &= ~bits;\
                                                        RegPBPU &=(~bits);\
                                                        RegPBPD &=(~bits);\
                                                        RegPBInpE &=(~bits)

#define PB_SetAnalog_with_PUP(bits)       RegPBOE   &= ~bits;\
                                                       RegPBPU |=(bits);\
                                                       RegPBInpE &=(~bits)

#define PB_SetAnalog_with_PDWN(bits)        RegPBOE   &= ~bits;\
                                                        RegPBPU &=(~bits);\
                                                        RegPBPD |=(bits);\
                                                    RegPBInpE &=(~bits)
...
#define PC_SetIN(bits)          RegPCOE   &= ~bits;\
                                          RegPCInpE |= bits
...
// Use following for analog IN/OUT
#define PC_SetAnalog(bits)                  RegPCOE   &= ~bits;\
                                                        RegPCPU &=(~bits);\
                                                        RegPCPD &=(~bits);\
                                                        RegPCInpE &=(~bits)

#define PC_SetAnalog_with_PUP(bits)       RegPCOE   &= ~bits;\
                                                       RegPCPU |=(bits);\
                                                       RegPCInpE &=(~bits)

#define PC_SetAnalog_with_PDWN(bits)        RegPCOE   &= ~bits;\
                                                        RegPCPU &=(~bits);\
                                                        RegPCPD |=(bits);\
                                                    RegPCInpE &=(~bits)
